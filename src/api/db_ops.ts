...
async function getAuthID(tweet_id: string){
...
async function onSend(address_from: string, value: number, tweet_id: string){
    // Register address[msg.sender], value, chain, 
    const _authID = await getAuthID(tweet_id)

    let {data, error} = await admin_client.from('tip_to_tweet').insert([{
